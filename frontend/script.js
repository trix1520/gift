...
const API_URL = (() => {
    ...
    // Для продакшена на Render - используем переменную окружения
    if (window.location.hostname.includes('onrender.com')) {
        // Если фронтенд и бэкенд на разных сервисах
        return 'https://giftmarket-backend.onrender.com/api';
    }
...
// Проверка загрузки translations.js
if (typeof translations === 'undefined') {
    console.error('❌ translations.js не загружен!');
    // Создаем заглушку для переводов
    window.translations = {
        ru: {}, 
        en: {}
    };
...
async function initUser() {
    ...
    try {
        console.log('👤 Инициализация пользователя:', telegramId);
        ...
        userData.username = user.username || 'Пользователь';
        userData.isAdmin = false;
        userData.requisites = {
            tonWallet: user.ton_wallet || null,
            card: user.card_number || null,
            cardBank: user.card_bank || null,
            cardCurrency: user.card_currency || null,
        ...
        // Создаем локального пользователя для офлайн режима
        userData.id = 1;
...
async function loadUserOrders() {
    try {
        console.log('📦 Загрузка ордеров пользователя...');
        ...
        orders = data || [];
        console.log(`✅ Загружено ${orders.length} ордеров`);
...
function updateUserInterface() {
    if (!userData || !userData.requisites) return;
    
    try {
        // TON кошелек
        if (userData.requisites.tonWallet) {
            const statusEl = document.getElementById('tonStatus');
            const displayEl = document.getElementById('tonWalletDisplay');
            const formEl = document.getElementById('tonWalletForm');
            ...
            if (addressEl) addressEl.textContent = userData.requisites.tonWallet;
        ...
        if (userData.requisites.telegram) {
            ...
            const displayEl = document.getElementById('telegramDisplay');
            ...
            if (usernameEl) usernameEl.textContent = userData.requisites.telegram;
            if (displayEl) displayEl.style.display = 'block';
            if (formEl) formEl.style.display = 'none';
        }
...
function editTonWallet() {
    const displayEl = document.getElementById('tonWalletDisplay');
    const formEl = document.getElementById('tonWalletForm');
    const inputEl = document.getElementById('tonWalletInput');
    
    if (displayEl) displayEl.style.display = 'none';
    if (formEl) formEl.style.display = 'block';
    if (inputEl && userData.requisites.tonWallet) {
        inputEl.value = userData.requisites.tonWallet;
    }
}
...
function editTelegram() {
    const displayEl = document.getElementById('telegramDisplay');
    const formEl = document.getElementById('telegramForm');
    const inputEl = document.getElementById('telegramInput');
    
    if (displayEl) displayEl.style.display = 'none';
    if (formEl) formEl.style.display = 'block';
    if (inputEl && userData.requisites.telegram) {
        inputEl.value = userData.requisites.telegram;
    }
}
...
function setupOrderCreation() {
    ...
    if (createBtn) {
        createBtn.addEventListener('click', showCreateOrderForm);
    }
    ...
    const submitBtn = document.getElementById('createOrderSubmit');
    if (submitBtn) {
...
function showCreateOrderForm() {
    ...
    const formEl = document.getElementById('createOrderForm');
...
function resetOrderForm() {
    ...
    const descInput = document.getElementById('orderDescription');
...
async function createOrder() {
    ...
    if (currentOrderData.payment === 'ton') {
        ...
        currency = userData.requisites.cardCurrency || 'RUB';
    ...
    try {
        ...
        if (response.ok) {
            ...
            throw new Error(error.error || 'Ошибка создания ордера');
        }
    } catch (error) {
...
function updateOrdersList() {
    ...
    if (!container || !listElement || !formElement) return;
    ...
    if (!orders || orders.length === 0) {
        ...
        const emptyTitle = container.querySelector('h3[data-i18n="noOrdersTitle"]');
        const emptyDesc = container.querySelector('p[data-i18n="noOrdersDesc"]');
        const emptyBtn = container.querySelector('button[data-i18n="createNewOrder"]');
        
        if (emptyTitle) emptyTitle.textContent = t('noOrdersTitle');
        if (emptyDesc) emptyDesc.textContent = t('noOrdersDesc');
        if (emptyBtn) {
            emptyBtn.textContent = t('createNewOrder');
            emptyBtn.onclick = showCreateOrderForm;
        }
    } else {
...
function createOrderCard(order) {
    ...
    if (userData && userData.isAdmin && order.status === 'active' && order.buyer_id) {
    ...
    return `<div class="order-card">
            ...
            <div class="order-details">
                <div class="order-detail">
                    ...
                    <span class="detail-value">${typeNames[order.type] || order.type}</span>
                </div>
                <div class="order-detail">
                ...
                ${order.buyer_id === (userData && userData.id) && order.status === 'active' ? `
                <div class="order-detail">
                    <span class="detail-label">${t('requisitesLabel')}</span>
                    <span class="detail-value">${order.seller_requisites}</span>
                </div>` : ''}
            </div>
            ${order.seller_id === (userData && userData.id) ? `<div class="order-link">
...
async function confirmTransfer(orderId) {
    ...
    console.log('📦 Продавец подтверждает передачу актива:', orderId);
...
async function actuallyConfirmTransfer(orderId) {
    ...
    // await fetch(`${API_URL}/orders/${orderId}/transfer`, { method: 'POST' });
}
...
async function confirmReceipt(orderId) {
    ...
    try {
        console.log('📦 Подтверждение получения актива:', orderId);
        ...
        console.error('❌ Ошибка confirmReceipt:', error);
...
function startDealsHistory() {
    ...
    // Добавляем новые сделки периодически
...
function updateProfileStats() {
    const completedEl = document.getElementById('completedDeals');
    const volumeEl = document.getElementById('totalVolume');
    const currencyStats = document.getElementById('currencyStats');
    
    if (completedEl) {
        completedEl.textContent = userData.stats.completedDeals || 0;
    }
    ...
    if (volumeEl) {
        volumeEl.textContent = '$' + totalVolumeUSD.toFixed(2);
    }
...
function addVolume() {
    ...
    if (parts.length === 2) {
        ...
        showToast(t('error'), 'Формат: CURRENCY:amount', 'error');
    }
}
...
async function checkNotifications() {
    ...
    try {
        ...
        if (notifications && notifications.length > 0) {
            ...
            if (unread.length > 0) {
                console.log(`🆕 Новых уведомлений: ${unread.length}`);
...
async function checkOrderFromUrl() {
    ...
    if (orderCode) {
        console.log('🔗 Найден ордер в URL:', orderCode);
        ...
        try {
            ...
            if (response.ok) {
                const order = await response.json();
                console.log('📦 Загружен ордер:', order);
...
async function showBuyerView(order) {
    ...
    showModal(`${t('orderCode')}${order.code}`, `
        <div class="modal-info-box">
            <p><strong>${t('type')}</strong> ${typeNames[order.type] || order.type}</p>
            <p><strong>${t('amount')}</strong> ${order.amount} ${order.currency}</p>
...
async function joinOrder(orderId) {
    ...
    try {
        ...
        console.error('❌ Ошибка joinOrder:', error);
...
window.editTonWallet = editTonWallet;
window.saveCard = saveCard;
window.editCard = editCard;
window.saveTelegram = saveTelegram;
window.editTelegram = editTelegram;
window.showCreateOrderForm = showCreateOrderForm;
window.cancelOrderCreation = cancelOrderCreation;
...
window.actuallyConfirmTransfer = actuallyConfirmTransfer;
window.confirmReceipt = confirmReceipt;
window.updateDealsCount = updateDealsCount;
window.addVolume = addVolume;
window.closeModal = closeModal;
window.joinOrder = joinOrder;
